...
export default function OrderDetails() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:1337/api/order/order-details/${id}`, {
      headers: {
          ...
          return (
            <div key={orderDetail.id} style={{float: 'left', paddingLeft: '2rem'}}>
              ...
              <CardInfo>
                ...
                <hr></hr>
                <h5>Quantity: {orderDetail.attributes.quantity}</h5>
                <h5>Price: ${orderDetail.attributes.product.data.attributes.price}</h5>
                <h5>Total: ${orderDetail.attributes.product.data.attributes.price * orderDetail.attributes.quantity}</h5>
              </CardInfo>
            </div>
          );
          })}
